...
public class FXMLController {
    ...
    void doSpellCheck(ActionEvent event) {
    	...
    	//2.Leggo la lingua selezionata e creo il dizionario associao
    	String language = buttonMenuTendina.getValue();
    	dizionario = new Dictionary();
    	dizionario.localDictionary(language);
    	
    	//3.Leggo il testo digitato, lo filtro e chiamo il metodo che mi restituisce le parole errate
    	...
    	List<String> listaParole = creaListaDiParole(filtrata);
    	...
    	int errori = 0;
    	//Se la struttura dati è vuota, allora ci sono parole errate, altrimenti è tutto ok
    ...
    private List<String> creaListaDiParole(String testoDigitato) {
		...
		while(testoDigitato.contains(" ")) {
			String parola = testoDigitato.substring(0, testoDigitato.indexOf(" "));
			l.add(parola);
			testoDigitato=testoDigitato.substring(parola.length(),testoDigitato.length());		
		}
